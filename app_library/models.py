...
def init_db(
    initial_book_records: List[Dict], initial_author_records: List[Dict]
) -> None:
    with _get_connection() as conn:
        cursor = conn.cursor()

        # СОЗДАЕМ И НАПОЛНЯЕМ ТАБЛИЦУ АВТОРОВ (AUTHORS)
        cursor.execute(
            f"""
        ...
        # СОЗДАЕМ И НАПОЛНЯЕМ ТАБЛИЦУ КНИГ (BOOKS)
        cursor.execute(
            f"""
            SELECT name FROM sqlite_master
        ...
        if not exists:
            ...
            cursor.executemany(
                ...
                """,
                [(item["title"], item["author_id"]) for item in initial_book_records],
            )
