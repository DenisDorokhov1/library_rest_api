...
authors_ns = api.namespace(
    "authors", description="Authors operations", path="/api/authors"
)
...
book_model = api.model(
    "Book",
    {
        ...
        "author_id": fields.Integer(
            required=True, description="Author ID, foreign key to Author.ID"
        ),
    },
)
...
author_create_model = api.model(
    "AuthorCreate",
    {
        "first_name": fields.String(
            required=True, example="Joanne", description="Required field"
        ),
        "last_name": fields.String(
            required=True, example="Rowling", description="Required field"
        ),
        # Поле middle_name НЕ имеет required=True, поэтому оно НЕ обязательное
        "middle_name": fields.String(
            required=False, example="Kathleen", description="Optional field"
        ),
    },
)
error_message = api.model(
    "Error", {"message": fields.String(example=["Book not found"])}
)
...
class BookList(Resource):
    ...
    def post(self) -> tuple[dict, int]:
        ...
        if not get_author_by_id(book["author_id"]):
            return {
                "message": f"There is not author in the DB with id {book['author_id']}"
            }, 404
        book = add_book(book)
...
class AuthorsList(Resource):
    ...
    @authors_ns.expect(author_create_model)
    ...
    def post(self) -> tuple[dict, int]:
        ...
        if result:
            return {
                "message": f"{author['first_name'], author['last_name']} already exists in the DB"
            }, 409
...
class Authors(Resource):
    ...
    def get(self, author_id) -> tuple[dict, int] | tuple[list[dict], int]:
        ...
        if result:
            ...
            return {
                "message": f"Author with id {author_id} has no books in the DB"
            }, 404
    ...
    def delete(self, author_id) -> tuple[dict, int]:
        ...
        if not author:
            return {
                "message": f"Author with id {author_id} is not found in the DB"
            }, 404

        delete_author_by_id(author_id)
        return {
            "message": f"Author with id {author_id} was deleted from the table as well as their books"
        }, 201
